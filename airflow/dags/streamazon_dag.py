...
EVENTS = ['amazon_sales']  # Amazon sales data event

GCP_PROJECT_ID = os.environ.get('GCP_PROJECT_ID')
GCP_GCS_BUCKET = os.environ.get('GCP_GCS_BUCKET')
BIGQUERY_DATASET = os.environ.get('BIGQUERY_DATASET', 'streamazon_stg')
...
with DAG(
    dag_id='streamazon_dag',
    default_args=default_args,
    description='Hourly data pipeline to generate dims and facts for Amazon Sales',
    schedule_interval="5 * * * *",  # At the 5th minute of every hour
    start_date=datetime(2026, 1, 21, 0),
    catchup=False,
    max_active_runs=1,
    user_defined_macros=MACRO_VARS,
    tags=['streamazon', 'amazon_sales']
) as dag:
    ...
    # Run dbt models to create dimension and fact tables
    execute_dbt_task = BashOperator(
        task_id='dbt_streamazon_run',
